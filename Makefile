...
usage:
	...
	"make run-migrations   # run all migrations in the webapp inside the web layer: make run-migrations\n" \
	"make restart-layer    # restart a specific layer: make restart-layer layer=layer\n" \
...
# USAGE
# run migrations on the web layer
# make run-migrations
run-migrations:
	docker-compose exec web bin/rails db:migrate

# USAGE
# restart a specific layer, must provide a layer:
# make restart-layer layer=web
restart-layer:
	docker-compose stop $(layer) \
	&& docker-compose up -d $(layer)